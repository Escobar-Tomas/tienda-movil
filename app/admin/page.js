  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-10 font-sans text-slate-900 selection:bg-indigo-100">
      ...
      <main className="p-4 space-y-6 max-w-4xl mx-auto -mt-2">
        ...
        {/* SECCIÓN 1: TARJETAS DE MÉTRICAS (KPIs) ajustada a 3 columnas */}
        {cargando ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 animate-pulse">
            {[1,2,3].map(i => <div key={i} className="bg-slate-200 h-24 rounded-2xl"></div>)}
          </div>
        ) : metricas && (
        ...
        <section className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <a href="/admin/ventas/historial" className="bg-rose-50 hover:bg-rose-600 text-rose-700 hover:text-white p-4 rounded-2xl flex flex-col items-center justify-center transition-all group cursor-pointer border border-rose-100 hover:border-transparent shadow-sm">
              <span className="text-2xl mb-2 group-hover:scale-110 transition-transform">📋</span>
              <span className="text-[10px] font-black uppercase tracking-widest text-center">Historial y<br/>Devoluciones</span>
            </a>
